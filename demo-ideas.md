...
as it comes in, which is what the arcade does too. Each size is the pilot's
...
depth, because a shallow board is the deep one with scanlines left out and
the depths on the screen are a subset of the deep board's. And it is planted
beside where the camera *will* be when it arrives: the camera pans ±900
...
road*, which gets repainted anyway. The stripes were later measured at
11,762, so it loses by more than two to one. Worse, it cannot be had: the
fill holds `SP` on the screen, so an interrupt would push its return address
into the middle of the road. Any routine in this repo that flips the palette
per scanline has that conflict, `chequer` included.

**What replaces it is not repainting what did not change.** A road's row is
one interval, so the window is the road plus a margin. The margin has to
cover *two* frames of movement, because that is how long a buffer waits its
turn: eight pixels at 20 world units a frame, which is a speed limit as much
as a margin.
...
**A road wider than the screen breaks both ends of that run**, and they want
opposite answers. A `PUSH` run cannot be started late, so the right hand end
